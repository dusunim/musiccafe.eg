...
const transcriptScriptOutput = path.join(root, 'content', 'transcripts.js');
...
const lessons = await Promise.all(course.sections.flatMap((section) => section.lessons.map(async (lesson) => {
  ...
  const transcriptPath = path.join(root, 'content', 'transcripts', ...lesson.file.replace(/\.mp4$/i, '.json').split('/'));
  try {
    const transcriptData = JSON.parse(await readFile(transcriptPath, 'utf8'));
    transcript = transcriptData.segments;
  } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }
  const id = `section-${section.number}-lesson-${lesson.number}`;
  if (transcript) transcriptMap[id] = transcript;
  return {
    ...lesson,
    id,
...
await Promise.all([
  ...
  writeFile(transcriptScriptOutput, `window.COURSE_TRANSCRIPTS = ${JSON.stringify(transcriptMap)};\n`, 'utf8'),
]);
